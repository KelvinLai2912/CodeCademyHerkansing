...
import codecademyher.database.CertificaatDB;
...
public class CertificaatView extends View {
    ...
    // aanmaken van alle nodige databases, een tabel met alle certificaat gegevens,
    // en een lijst met alle certificaat objecten
    ...
    public CertificaatView(CertificaatDB cerdb, ObservableList<Certificaat> certificaat, InschrijvingDB idb,
            ObservableList<Inschrijving> inschrijving) {
        this.cerdb = cerdb;
    ...
    public Stage getScene() throws SQLException {
        ...
        // als de gebruiker op de create button drukt wordt de createCertificaat methode
        // aangeroepen
        ...
        // als de gebruiker een certificaat selecteert uit de tabel en op de delete
        // button drukt wordt de certificaat uit de certificaat database verwijderd
        ...
        // als de gebruiker een certificaat selecteert uit de tabel en op de update
        // button drukt wordt de editCertificaat methode aangeroepen
        // deze methode geeft een stage terug en deze wordt getoond aan de gebruiker
        Button update = new Button("Update");
        update.setOnAction((e) -> {
            try {
                Stage deleteWindow = editCertificaat(
                        (Certificaat) certificaatTable.getSelectionModel().getSelectedItem());
    ...
    // methode die alle certificaat gegevens verwijderd en opnieuw de
    // getAllCertificaten methode aanroept in de certificaten database
    ...
    // nieuw certificaat kan aanmaken
    public Stage createCertificaat() {
        ...
        // create button sluit de stage af, en maakt een nieuw certificaat in de
        // certificaat database aan met de gegevens uit de input velden
        create.setOnAction((e) -> {
    ...
    // methode returnt een stage waarop de gebruiker gegevens van een certificaat
    // kan wijzigen
    public Stage editCertificaat(Certificaat c) {
        ...
        // update button sluit de stage af, en maakt een nieuw certificaat aan en deze
        // vervangt het oude certificaat in de certificaat database
        update.setOnAction((e) -> {
            if (val.isValidCijfer(cijferField.getText())) {
                Certificaat newC = new Certificaat(
                        0,
        ...
        layout.getChildren().addAll(cijferField, medewerkerField, certificaatField, inschrijvingFieldLabel,
                inschrijvingField, update);
