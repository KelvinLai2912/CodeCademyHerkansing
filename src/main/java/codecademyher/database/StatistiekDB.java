package codecademyher.database;
...
/**
 * StatistiekDB beheert de informatie uit de database die bestemd zijn voor de
 * statistiekoverzichten uit de opdracht.
 */
public class StatistiekDB {
    ...
    // Retourneert als double het percentage van de inschrijvingen waar een
    // certificaat voor is behaald voor het geslacht in de parameters.
    ...
    // per module van een bepaalde cursus, als HashMap<ContentItemID,
    // GemiddeldeVoortgang>
    public HashMap<Integer, Double> gemiddeldeVoortgangPerModulePerCursus(Cursus c) {
        ...
        String SQL = String.format(
                "SELECT Module.ContentItemID, SUM(Voortgang) / ((SELECT COUNT(*) FROM Inschrijving WHERE Inschrijving.CursusID=%d)) AS GemVoortgang\n"
                        +
    ...
    // Retourneert de voortgang per module van een cursist voor een cursus, als
    // Hashmap <ContentItemID, Percentage>
    ...
    // Retourneert alle certificaten die een bepaalde cursist bezit in een
    // ArrayList.
    ...
    // Retourneert de top 3 cursussen die het meeste certificaten hebben in een
    // ArrayList.
    ...
    // Retourneert als int de hoeveelheid certificaten die zijn uitgegeven bij een
    // bepaalde cursus.
