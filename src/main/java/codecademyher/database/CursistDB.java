...
/**
 * CursistDB beheert de informatie uit de database voor gegevens uit de
 * 'Cursist' tabel.
 */
public class CursistDB {

    // Maakt voor elke record in de 'Cursist' tabel een Cursist en retourneert deze
    // in een ArrayList.
    ...
    public void addCursist(Cursist c) {
        String addCursist = String.format(
                "INSERT INTO Cursist VALUES (\'%s\',\'%s\',\'%s\',\'%s\',\'%s\',\'%s\',\'%s\',\'%s\',\'%s\')",
                c.getEmailAdres(),
                c.getNaam(),
    ...
    public void updateCursist(Cursist oldC, Cursist newC) {
        String updateCursist = String.format("UPDATE Cursist SET "
                ...
                + "Woonplaats=\'%s\',"
                + "Land=\'%s\'"
                + "WHERE ID=%d", newC.getEmailAdres(), newC.getNaam(), newC.getGeboortedatum().toString(),
                newC.getGeslacht().getGeslachtNaam(),
                newC.getStraat(), newC.getHuisnr(), newC.getPostcode(), newC.getWoonplaats(), newC.getLand(),
                oldC.getId());
