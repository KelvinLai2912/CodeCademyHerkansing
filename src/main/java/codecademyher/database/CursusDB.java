...
/**
 * CursusDB beheert de informatie uit de database voor gegevens uit de 'Cursus'
 * tabel.
 */
public class CursusDB {

    // Maakt voor elke record in de 'Cursus' tabel een Cursus en retourneert deze in
    // een ArrayList.
    ...
    public boolean updateCursus(Cursus oldC, Cursus newC) {
        String updateCursus = String.format("UPDATE Cursus SET "
                ...
                + "IntroductieTekst=\'%s\',"
                + "NiveauAanduiding=\'%s\'"
                + "WHERE ID=%d", newC.getNaam(), newC.getOnderwerp(), newC.getIntroductietekst(),
                newC.getNiveauaanduiding().getNiveauNaam(), oldC.getId());
