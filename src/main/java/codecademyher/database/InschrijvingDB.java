...
/**
 * InschrijvingDB beheert de informatie uit de database voor gegevens uit de
 * 'Inschrijving' tabel.
 */
public class InschrijvingDB {
    ...
    // Maakt voor elke record in de 'Inschrijving' tabel een Inschrijving en
    ...
    public void updateInschrijving(Inschrijving oldI, Inschrijving newI) {
        String updateInschrijving = String.format("UPDATE Inschrijving SET "
                ...
                + "CursistID=%d,"
                + "Datum=\'%s\'"
                + "WHERE ID=%d", newI.getCursus().getId(), newI.getCursist().getId(), newI.getDatum().toString(),
                oldI.getId());
