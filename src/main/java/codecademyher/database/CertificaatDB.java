...
/**
 * CertifaatDB beheert de informatie uit de database voor gegevens uit de
 * 'Certificaat' tabel.
 */
public class CertificaatDB {
    ...
    // Maakt voor elke record in de 'Certificaat' tabel een Certificaat en
    // retourneert deze in een ArrayList.
    ...
    public void updateCertificaat(Certificaat oldC, Certificaat newC) {
        String updateCertificaat = String.format(Locale.US, "UPDATE Certificaat SET "
                ...
                + "CertificaatNummer=%d,"
                + "InschrijvingID=%d"
                + "WHERE ID=%d", newC.getCijfer(), newC.getMedewerker(), newC.getNummer(),
                newC.getInschrijving().getId(), oldC.getId());
